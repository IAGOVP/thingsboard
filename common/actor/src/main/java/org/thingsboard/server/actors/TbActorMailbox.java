...
/**
 * Single actor's mailbox: two priority queues, init lifecycle, and serialized {@link TbActor#process}
 * on the bound {@link Dispatcher} executor.
 *
 * <p>Also implements {@link TbActorRef} / {@link TbActorCtx} so the actor can tell itself, children, or siblings via {@link TbActorSystem}.
 */
@Slf4j
@Getter
...
public void initActor() {
    ...
    /** Runs actor init; on failure applies {@link TbActor#onInitFailure(int, Throwable)}. */
    ...
    /** Adds to queue and triggers processing; handles destroy and rule-node update edge cases. */
    ...
    /** Drains up to {@link TbActorSystemSettings#getActorThroughput()} messages per scheduler tick. */
