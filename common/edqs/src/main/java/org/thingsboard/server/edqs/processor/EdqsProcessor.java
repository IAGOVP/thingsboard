...
public class EdqsProcessor implements TbQueueHandler<TbProtoQueueMsg<ToEdqsMsg>, TbProtoQueueMsg<FromEdqsMsg>> {
    ...
    /** Consumer for entity change events ({@link EdqsConfig#getEventsTopic()}). */
    private PartitionedQueueConsumerManager<TbProtoQueueMsg<ToEdqsMsg>> eventConsumer;
    /** Request-reply handler for {@link EdqsRequest} from core services. */
    private PartitionedQueueResponseTemplate<TbProtoQueueMsg<ToEdqsMsg>, TbProtoQueueMsg<FromEdqsMsg>> responseTemplate;
    /** Offloads query handling from the Kafka poll thread. */
    private ListeningExecutorService requestExecutor;
    /** Drops out-of-order entity updates using event version numbers. */
    private VersionsStore versionsStore;
    private final AtomicInteger counter = new AtomicInteger();

    @Getter
    /** Fatal error callback; closes the app on {@link OutOfMemoryError}. */
    private Consumer<Throwable> errorHandler;

    /** Starts Kafka consumers and registers partition/state hooks. */
    @PostConstruct
    private void init() {
        errorHandler = error -> {
    ...
    */
    ...
    /** Dispatches to {@link EdqsRepository#findEntityDataByQuery} or {@link EdqsRepository#countEntitiesByQuery}. */
