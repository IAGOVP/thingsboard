...
/**
 * Streams a PostgreSQL {@code pg_dump} file and writes ThingsBoard telemetry rows
 * as Cassandra SSTables via {@link CQLSSTableWriter}.
 *
 * <p>Recognizes {@code COPY public.ts_kv_latest} and {@code COPY public.ts_kv} blocks.
 * Tab-separated rows are mapped to Cassandra column order; optional {@code castEnable}
 * moves numeric strings from {@code str_v} to {@code dbl_v}. Partition keys are
 * collected during TS migration and flushed to {@code ts_kv_partitions_cf} at the end.
 *
 * <p>Writers are rotated every one million rows to limit SSTable file size.
 */
public class PgCaMigrator {

    /** Log progress every N lines processed or migrated. */
    private final long LOG_BATCH = 1000000;
    /** Close and reopen SSTable writer after this many rows per output directory. */
    ...
    /**
     * @param sourceFile PostgreSQL dump containing {@code ts_kv} / {@code ts_kv_latest} COPY blocks
     * @param ourTsDir output dir for {@code ts_kv_cf} SSTables (null to skip historical TS)
     * @param outTsPartitionDir output dir for partition SSTables (required if {@code ourTsDir} set)
     * @param outTsLatestDir output dir for {@code ts_kv_latest_cf} (null to skip latest)
     * @param allEntityIdsAndTypes entity UUID → type name from related-entities dump
     * @param dictionaryParser key id → telemetry key string from same or related dump
     * @param castStringsIfPossible when true, promote numeric {@code str_v} to {@code dbl_v}
     */
    ...
    /**
     * Single pass over {@link #sourceFile}: migrates latest block first (if writer configured),
     * then historical {@code ts_kv}, then writes collected partition keys.
     *
     * @throws IOException if dump read or SSTable write fails
     */
