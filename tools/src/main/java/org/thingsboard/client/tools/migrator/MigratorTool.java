...
/**
 * CLI entry point for PostgreSQL telemetry dump → Cassandra SSTable migration.
 *
 * <p>Parses Apache Commons CLI options, builds {@link RelatedEntitiesParser} and
 * {@link DictionaryParser} from dump files, then runs {@link PgCaMigrator#migrate()}.
 *
 * <p>Required: {@code -telemetryFrom}, {@code -relatedEntities}, {@code -castEnable}.
 * Optional output dirs: {@code -telemetryOut} + {@code -partitionsOut}, {@code -latestTelemetryOut}.
 *
 * @see org.thingsboard.client.tools.migrator README and {@code tools/CLI.md}
 */
public class MigratorTool {

    /**
     * Parses CLI args, wires parsers and migrator, prints progress to stdout.
     *
     * @param args see {@link #parseArgs(String[])} for option names
     */
    ...
    /**
     * Defines and parses migrator CLI options; prints help and exits with code 1 on parse failure.
     *
     * @param args raw command-line arguments
     * @return parsed {@link CommandLine} (never null unless process exits)
     */
