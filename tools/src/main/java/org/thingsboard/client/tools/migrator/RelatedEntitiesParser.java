...
/**
 * Indexes entity UUID → {@link EntityType#name()} from multiple {@code COPY public.<table>}
 * blocks in a PostgreSQL entities dump (devices, assets, tenants, etc.).
 */
public class RelatedEntitiesParser {
    /** entity id (UUID string) → entity type name (e.g. DEVICE) */
    private final Map<String, String> allEntityIdsAndTypes = new HashMap<>();

    /** COPY line prefix → entity type for first-column UUID rows */
    ...
    /**
     * @param source SQL dump with COPY blocks for supported entity tables
     */
    ...
    /**
     * @param uuid entity id from telemetry row (first tab field)
     * @return {@link EntityType} name string, or null if not found in dump
     */
